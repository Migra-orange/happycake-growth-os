  ...
  try {
    ...
    return res.status(200).json({
      ...
      agents: [
        ...
        { id: 'promo-engine', name: 'Promo engine', enabled: true, mode: 'suggest_only', tone: 'playful', dailyLimit: 12, goal: 'Run wheel offers, office bundles, and comeback cards without fake promises.' },
        { id: 'owner-approval', name: 'Owner approval router', enabled: true, mode: 'telegram_first', tone: 'concise', dailyLimit: 120, goal: 'Send clean approval cards and block risky handoffs.' },
    ...
    return res.status(200).json({ ok: true, mode: 'simulated', updatedAt: new Date().toISOString(), metrics: {}, funnel: [], channels: [], topProducts: [], mcpChecks: [], agents: [] });
  }
}
