...
| Requirement | Proof |
| --- | --- |
| Claude Code CLI runtime | Local/live Node proof: `src/server/assistant.ts` shells to `claude -p` and fails closed in `ASSISTANT_MODE=live`. Production `/api/health` intentionally separates `localRuntime: "claude-code-cli"` from `productionRuntime: "vercel_serverless_mcp_demo"` and sets `claudeCliProductionExecuted: false`, because Vercel functions do not run the local Claude Code CLI binary. No Claude Agent SDK, LangGraph, CrewAI, n8n, or alternate core LLM provider is used. |
